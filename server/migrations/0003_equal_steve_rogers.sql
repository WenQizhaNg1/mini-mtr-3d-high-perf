CREATE TABLE "mtr"."motion_checkpoint" (
	"id" integer PRIMARY KEY NOT NULL,
	"state" jsonb NOT NULL,
	CONSTRAINT "motion_checkpoint_singleton" CHECK ("mtr"."motion_checkpoint"."id" = 1)
);
--> statement-breakpoint
CREATE OR REPLACE FUNCTION mtr.train_positions(p_at timestamptz)
RETURNS TABLE (
    id text,
    line_id text,
    pattern_id text,
...
AS $function$
    WITH active_runs AS (
        SELECT
            run.id,
            run.pattern_id,
            0::double precision AS delay_seconds
        FROM mtr.train_runs AS run
        WHERE p_at >= run.starts_at
          AND p_at <= run.ends_at
    ),
    leg_windows AS (
        SELECT
            leg.*,
